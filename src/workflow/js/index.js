...
(function () {
  ...
  function syncDevDocEditToolbar() {
    const importRow = $('wf-devdocs-detail-import-row');
    ...
    if (importRow) importRow.hidden = !devDocEditing || !showImport;
    if (pickBtn) pickBtn.hidden = mode !== 'file';
    if (urlInput) {
      urlInput.placeholder = mode === 'url' ? 'https://…' : '文件路径';
    }
  }
  ...
  function renderSkillLayerTree(container, tree, skillName) {
    if (!container) return;
    const nodes = tree?.nodes || [];
    if (!nodes.length) {
      container.innerHTML = '<p class="wf-skill-tree-empty">暂无 Layer 树，生成 skill 后将自动生成。</p>';
      return;
    }
  ...
  async function saveSavedSkillViaMarkdownPath() {
    ...
    const res = await getApi()?.workflowDevDocsGenerateSkill?.({
      id: activeDevDocId,
  ...
  function bindUi() {
    ...
    mount.querySelectorAll('.workflow-tab').forEach((btn) => {
      btn.addEventListener('click', () => switchTab(btn.dataset.wfTab || 'devdocs'));
    });
    ...
    $('wf-devdocs-edit')?.addEventListener('click', () => {
      if (!activeDevDocId) return;
      if (activeDevDoc) resetDevDocEditDrafts(activeDevDoc);
      setDevDocEditMode(true);
    });
    $('wf-devdocs-edit-source-switch')?.querySelectorAll('[data-source]').forEach((btn) => {
      btn.addEventListener('click', () => {
