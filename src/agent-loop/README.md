...
- **有 tool_calls**：执行 tools 后：
  - 若含 **`write_file` / `edit_file`**：写入 `conv` 后 **不再进入下一轮 INFER**；应用 **自动 `xcode_build`**（`post-write-xcode.js`），可选在用户有运行意图时 **自动 `xcode_run`**，合并为一条 `content` 返回。
  - 若仅 **`xcode_build` / `xcode_run` / `xcode_test`**（用户主动要求编译/运行/测试）：执行完即返回，不再下一轮 LLM。
...
| 旧行为 | 新行为 |
|--------|--------|
| 写代码后模型常再调 `xcode_build` → 再 `xcode_run` → 多轮 LLM | 写代码后 **1 轮 LLM**，build/run 由应用本地执行 |
| 每轮 tool 结果进 conv 后再 INFER | 写代码路径 **跳过** build 后的 INFER |
| Prompt 鼓励改 Swift 必 build+run | Prompt：写完后 **勿**调 build；**仅**用户要运行时才 `xcode_run` |

实现：`post-write-xcode.js`（自动编译、运行意图检测、回复拼装）+ `app-agent-loop.js`（写代码 / Xcode 工具后短路返回）。详见根目录 [README.md](../../README.md)「Token 消耗优化」。
...
| 文件 | 职责 |
|------|------|
| `app-agent-loop.js` | `runAppAgentLoop`：for 轮编排、MCP + Xcode tools 列表、写代码后短路 |
| `post-write-xcode.js` | 写代码后自动 `xcode_build`；运行意图 → 可选 `xcode_run` |
| `task-dispatcher.js` | `route_task`：`mcp_tool` → mcp-filesystem；`xcode_tool` → xcode |
